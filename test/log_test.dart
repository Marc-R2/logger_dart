...
void main() {
  const runtime123 = LogRuntime('123');
  const sessionAbc = LogSession('abc');
  group('Log', () {
    test('Test log method', () {
      ...
      expect(message.runtimeId.runtimeId, equals('123'));
    ...
    test('Test child method', () {
      ...
      expect(childLog.runtimeId, equals(runtime123));
      expect(childLog.sessionId, equals(sessionAbc));
    ...
    test('Test creating a message with a child template', () {
      final log = Log(tags: ['Test'], runtime: runtime123, session: sessionAbc);
      final message = log.log(title: 'Test message', message: 'Test');
      expect(message.tags, contains('Test'));
    });

    test('Test creating a message with a child template and class', () {
      final log = Log(klasse: 'Log', runtime: runtime123, session: sessionAbc);
      final message = log.log(title: 'Test message', message: 'Test');
    ...
    group('finish group', () {
      test('with no delay', () async {
        final log = Log(runtime: runtime123, session: sessionAbc);
    ...
    group('Log tests', () {
      test('Log runtime should be inherited from parent if not defined', () {
        final parent = Log(
          runtime: const LogRuntime('parent_session'),
          session: sessionAbc,
        );
        final child = Log(parent: parent);
        expect(child.runtime.runtimeId, equals('parent_session'));
      });

      test('Log tags should contain session and id', () {
        final log = Log(
          runtime: const LogRuntime('test_session'),
          session: sessionAbc,
        );
        expect(log.runtime.runtimeId, equals('test_session'));
      ...
      test('Log tags should not contain null session', () {
        ...
        // expect(log1.logId, equals(0));
        expect(log1.runtime.runtimeId, equals('parent_session'));
        // expect(log2.logId, equals(1));
        expect(log2.runtime.runtimeId, equals('parent_session'));
      });
    });
  });
}
